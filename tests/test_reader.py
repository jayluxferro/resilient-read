...
def test_read_bytes_and_cursor(tmp_path: Path) -> None:
    ...
    first = read_bytes(tmp_path, "big.txt", offset=0, max_bytes=100)
    assert first["ok"] is True
    assert first["bytes_read"] == 100
    assert first["next_offset"] == 100
    assert first["eof"] is False

    c = make_cursor(tmp_path, "big.txt", offset=100, max_bytes=120)
    nxt = read_next(tmp_path, c["cursor"])
...
def test_read_lines_window(tmp_path: Path) -> None:
    ...
    out = read_lines(tmp_path, "code.py", start_line=5, max_lines=4)
...
def test_read_tail(tmp_path: Path) -> None:
    ...
    tail = read_tail(tmp_path, "app.log", max_lines=3)
...
def test_search_then_page(tmp_path: Path) -> None:
    ...
    out = search_then_page(tmp_path, "mix.txt", query="needle", max_matches=1)
